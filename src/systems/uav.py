...
from multirotor.vehicle import (PropellerParams, VehicleParams, SimulationParams)
from multirotor.controller import Controller, AltController, AttController, PosController

from .base import SystemEnv



def get_controller(vehicle: Multirotor) -> Controller:
    return Controller(
        # (Outer) Position controller has low sensitivity
        PosController(0.5, 0., 0.5, 1., dt=vehicle.simulation.dt, vehicle=vehicle),
        # (Inner) Attitude controller is more responsive (except for yaw, which we are not controlling)
        AttController(
            np.asarray([25., 25., 2.]),
            np.asarray([0., 0., 0.]),
            np.asarray([5, 5, 0.2]),
            1., dt=vehicle.simulation.dt, vehicle=vehicle),
        # Altitude controller is more responsive as well
        AltController(50, 0, 0.5, 1, dt=vehicle.simulation.dt, vehicle=vehicle),
        interval=vehicle.simulation.dt * 1
    )



def get_controller_(vehicle: Multirotor) -> Controller:
    return Controller(
        # (Outer) Position controller has low sensitivity
        PosController(5., 0., 0., 1., dt=vehicle.simulation.dt, vehicle=vehicle),
        # (Inner) Attitude controller is more responsive (except for yaw, which we are not controlling)
        AttController(np.asarray([0.5, 0.5, 0.5]),
                    np.asarray([0., 0., 0.]),
                    np.asarray([0., 0., 0.]),
                    1., dt=vehicle.simulation.dt, vehicle=vehicle),
        # Altitude controller is more responsive as well
        AltController(10, 4, 5, 1, dt=vehicle.simulation.dt, vehicle=vehicle),
        interval=vehicle.simulation.dt * 1
    )


def create_multirotor(
    vp, sp, name='multirotor', xformA=np.eye(12), xformB=np.eye(4),
    return_multirotor=False
):
    m = Multirotor(vp, sp)
...
class MultirotorEnv(SystemEnv):
    ...
    def __init__(
        ...
        system, self.mult = create_multirotor(
            vp, sp, xformA=xformA, xformB=xformB,
            return_multirotor=True
        )
        super().__init__(system, q, sp.dt, seed, dtype=np.float32)
    ...
    def step(self, u: np.ndarray, unnormalize: bool=True, clip: bool=False):
...
class ControlledMultirotorEnv(MultirotorEnv):
    ...
    def step(self, u: np.ndarray):
        ...
        x, r, d, i = super().step(dynamics, unnormalize=False, clip=True)
        return self.state, r, d, {'u': u}
